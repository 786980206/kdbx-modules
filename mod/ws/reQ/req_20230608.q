...
.os.es:$[.z.o like "w*";" 2>NUL";" 2>/dev/null"];                                       //error suppression dependent on os
.os.test:{[x]
  /* .os.test - test if a command works on current os */
  :@[{system x;1b};x,.os.es;0b];                                                        //run with system & suppress error
  }

.os.home:hsym`$getenv$[.z.o like "w*";`USERPROFILE;`HOME]                               //get home dir depending on OS
.os.hfile:(` sv .os.home,)                                                                  //get file path relative to home dir

.os.read:{$[1=count a;first;]a:read0 x}                                                 //read text file, single string if one line
.os.write:{x 0:$[10=type y;enlist;]y}                                                   //write text file, list of strings or single

.os.hread:{read .os.hfile x}                                                                //read file from home dir
.os.hwrite:{.os.write[.os.hfile x;y]}                                                           //write file in home dir
...
.url.query:{[x]@["?"vs x;1;dec]}                                                         //split on ?, urldecode query
...
.url.parse0:{[q;x]
  ...
  :$[q;@[o;`path`query;:;.url.query o`path];o];                                          //split path into path & query if flag set, return
  }
...
.url.parse:.url.parse0[1b]                                                               //projection to parse query by default
...
.url.format:{[x]
  ...
  $[99=type x`query;"?",.url.enc x`query;""];                                            //if there's a query, encode & append
  }
...
.cookie.jar:([host:();path:();name:()] val:();expires:`datetime$();maxage:`long$();secure:`boolean$();httponly:`boolean$();samesite:`$())
...
.cookie.addcookies:{[q]
  if[count c:.cookie.getcookies[q`url];q[`headers;`Cookie]:c];
  :q;
  }
...
.b64.enc:{(neg[c] _ .Q.b6 0b sv' 00b,/:6 cut raze (0b vs'`byte$x),(8*c)#0b),(c:neg[count x]mod 3)#"="}
.b64.enc:@[value;`.Q.btoa;{[x;y]x}.b64.enc]
...
.b64.dec:{(`char$0b sv'8 cut raze 2_'0b vs'`byte$.Q.b6?x) except "\000"}
...
.status.class:{c:div[;100];$[0=type x;.z.s[first x];99=type x;c x`status;c x]}              //get class from status code, header dict or return object
...
addheaders:{[q]
  d:.z.m.def;
  if[count q[`url;`auth];d[$[`proxy in key q;"Proxy-";""],"Authorization"]:"Basic ",.b64.enc q[`url;`auth]]; // add proxy authorization header if present
  if[count q[`url;`auth];d["Authorization"]:"Basic ",.b64.enc q[`url;`auth]];       //add authorization header if present
...
formatresp:{[r]
  ...
  if[(`$"content-encoding")in key d;
      p[1]:.z.m.decompress[d`$"content-encoding"]p[1];                              //if compressed, decompress body based on content-encoding
    ];
...
okstatus:{[v;x]
  if[not[.z.m.SIGNAL]|v|x[0][`status] within 200 299;:x];                           //if signalling disabled, in verbose mode or OK status, return
...
send:{[m;u;hd;p;v]
  ...
  q:@[.z.m.query;`method`url`headers`body;:;(m;.url.parse0[0]u 0;hd;p)];            //parse URL into URL object & build query
  ...
  //hs:.url.hsurl`$raze q ./:enlist[`url`protocol],$[`proxy in key q;enlist 1#`proxy;enlist`url`host]; //get hostname as handle
  // fix: 代理问题, 修正获取代理错误的问题
  hs:.url.hsurl`$raze q ./:enlist[`url`protocol],$[`proxy in key q;enlist`url`host;enlist`url`host]; //get hostname as handle
...
parseresp:{[r]
  ...
  if[not .z.m.PARSE;:2#r];                                                          //if parsing disabled, return "raw" response (incl. headers dict)
  f:$[(`j in key`)&r[0][`$"content-type"]like .z.m.ty[`json],"*";.j.k;::];          //check for JSON, parse if so
...
// @qlintsuppress RESERVED_NAME
.z.m.get:{parseresp okstatus[.z.m.VERBOSE] send[`GET;x;y;();.z.m.VERBOSE]}
...
// @return {(dict;string)|any} HTTP response (headers;body), or parsed if JSON
.z.m.post:{parseresp okstatus[.z.m.VERBOSE] send[`POST;x;y;z;.z.m.VERBOSE]}
...
// @param z {string|any} body for HTTP request (if non-string, must be an encoder in .z.m.tx)
// @return {(dict;string)|any} HTTP response (headers;body), or parsed if JSON
.z.m.p:{[x;y;z]
  if[10h<>type z;
    if[not y in key .z.m.tx;'type];
    z:.z.m.tx[y] z;
  ];
  y:enlist["Content-Type"]!enlist .z.m.ty y;
  parseresp okstatus[.z.m.VERBOSE] send[`POST;x;y;z;.z.m.VERBOSE]}
...
// @return {(dict;string)|any} HTTP response (headers;body), or parsed if JSON
// @qlintsuppress RESERVED_NAME
.z.m.delete:{parseresp okstatus[.z.m.VERBOSE] send[`DELETE;x;y;z;.z.m.VERBOSE]}
...
.auth.getauth:{[h;u] /h-headers,u-URL
...
.auth.cache:([host:`$()]auth:();expires:`timestamp$())
...
// @return null
.auth.setcache:{[host;auth].auth.cache[`$host]:`auth`expires!(auth;.z.p+0D00:15:00)}
...
.auth.getcache:{[hst]
  r:exec first auth from .auth.cache where host=`$hst,expires>.z.p;
  if[count r;:r];
  if[.auth.netrcenabled;:.auth.readnetrc hst];
  :();
 }
...
.auth.netrcenabled:not()~key .os.hfile`.netrc

// @kind data
// @category public
// @fileoverview location of .netrc file, by default ~/.netrc
.auth.netrclocation:.os.hfile`.netrc
...
mkpt:{[b;n;v]
  ...
  if[f;t:"Content-Type: ",$[0<count t:.h.ty last` vs`$.url..url.sturl v;t;"application/octet-stream"],"\r\n"];   //get content-type for part
  r :"--",b,"\r\n";                                                                 //opening boundary
...
// @return {(dict;string)} (HTTP headers;body) to give to .z.m.post
multi:{[d]
